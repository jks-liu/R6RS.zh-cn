...
表可以只用一个改进的记号：表中的元素被简单地括进小括号中并用空格分隔。空表用`()`表示。比如：
...
表示向量对象（见11.13小节）的向量数据使用`#(<datum> ...)`表示。比如：一个长度是3，且0号元素位置和数字对象零，1号元素位置是表`(2 2 2 2)`，2号元素位置是字符串`"Anna"`，的向量可以如下表示：
...
表示字节向量（见库的第2章）的向量数据使用符号`#vu8(<u8> ...)`表示，其中`<u8>`表示字节向量的八位字节（octets）。比如：一个长度是3节包含八位字节2，24和123的字节向量可以如下表示：

`#vu8(2 24 123)`

这时一个字节向量的外部表示，也是一个计算结果为字节向量的表达式。
...
一个Scheme程序由一个*顶层程序*以及一系列的*库*组成，每个库定义了通过导入和导出显式地和其它程序相联系的程序的一部分。一个库由一系列的导入导出说明和程序体组成，程序体由定义和表达式组成。一个顶层程序和一个库差不多，但是没有导出说明。第7章和第8章分别描述了库和顶层程序的语法和语义。第11章描述了定义了通常和Scheme一起的结构的一个基本库。一个单独的报告[^24]描述了Scheme系统提供的各种标准库。
...
自一个库或顶层程序的内部，一个标识符可以命名一种语法，或者它可以命名一个值可以被存储的位置。命名一种语法的标识符叫做*关键字*，
